...
quality: setup-env quality-contract-tests quality-go-tests ## Run all quality checks and deterministic tests (ENV_MANAGER=micromamba|mise|system, LINT_MODE=fix|check)
	...
	@$(CMD_ECHO) "+ $(RUN) uv run pre-commit install --install-hooks"
	@$(RUN) uv run pre-commit install --install-hooks >/dev/null 2>&1 || true
	@$(CMD_ECHO) "+ LINT_MODE=$(LINT_MODE) $(RUN) uv run pre-commit run --all-files --color=always"
	@LINT_MODE=$(LINT_MODE) $(RUN) uv run pre-commit run --all-files --color=always
...
quality-actions: setup-env ## Opt-in GitHub Actions linting with actionlint and zizmor
	@$(CMD_ECHO) "+ $(RUN) uv run pre-commit run lint-actions --all-files --color=always"
	@$(RUN) uv run pre-commit run lint-actions --all-files --color=always
