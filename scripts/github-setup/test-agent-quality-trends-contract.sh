...
result="$($python_bin "$repo_root/templates/.github/config/audit-tests/test_quality_trends.py")" || {
    printf '{"schema_version":1,"result":"FAIL","error_code":"AUDIT_QUALITY_TEST_FAILED"}\n'
    exit 1
}
generated_result="$($python_bin "$generated_root/.github/config/audit-tests/test_quality_trends.py")" || {
    printf '{"schema_version":1,"result":"FAIL","error_code":"AUDIT_QUALITY_TEST_FAILED"}\n'
    exit 1
}
