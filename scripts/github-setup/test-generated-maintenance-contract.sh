...
assert_contains 'app_user_owner: ${{ inputs.app_owner }}' "$workflow"
assert_contains 'PR_AUTHOR_USER_TOKEN: ${{ secrets.BOOTSTRAP_E2E_FIXTURE_APP_USER_ACCESS_TOKEN }}' "$workflow"
...
assert_contains 'automation: opt-in' "$workflow"
...
assert_contains 'secret_writer_token: ${{ steps.provisioner-token.outputs.token }}' "$workflow"
...
assert_contains 'MAINTENANCE_FIXTURE_LOGIN: ${{ inputs.app_owner }}' "$workflow"
assert_contains 'MAINTENANCE_COPILOT_REVIEWER_LOGIN' "$workflow"
assert_contains 'wait_for_fixture_repository_access' "$workflow"
assert_contains 'fixture repository access' "$workflow"
assert_contains 'fixture branch creation' "$workflow"
assert_contains 'write_fixture_content' "$workflow"
assert_contains 'if commit_response="$(writer_gh api --method PUT "/repos/$OWNER/$REPO_NAME/contents/$path"' "$workflow"
assert_contains 'fixture commit signature' "$workflow"
assert_contains "--jq '[.commit.verification.verified,.commit.verification.reason] | @tsv'" "$workflow"
assert_not_contains 'if author_gh api --method PUT "/repos/$OWNER/$REPO_NAME/contents/$path"' "$workflow"
assert_contains 'TEMP_REF_CREATED=false' "$workflow"
assert_contains 'refs/heads/$DISPATCH_REF' "$workflow"
assert_contains "gh workflow run create-repository.yml --repo \"\$REPOSITORY\" \\" "$workflow"
assert_contains "--ref \"\$DISPATCH_REF\" \\" "$workflow"
assert_contains 'create_fixture_pull_request' "$workflow"
maintenance_started_at_line="$(grep -n 'maintenance_started_at=.*date -u' "$workflow" | head -1 | cut -d: -f1)"
fixture_pr_creation_line="$(grep -n 'PR_NUMBER=.*create_fixture_pull_request' "$workflow" | head -1 | cut -d: -f1)"
[ -n "$maintenance_started_at_line" ] || fail "maintenance lifecycle boundary timestamp is missing"
[ -n "$fixture_pr_creation_line" ] || fail "fixture PR creation is missing"
[ "$maintenance_started_at_line" -lt "$fixture_pr_creation_line" ] ||
    fail "maintenance lifecycle boundary must be captured before fixture PR creation"
assert_contains 'request_fixture_reviewer' "$workflow"
assert_contains 'writer_gh api --method POST "/repos/$OWNER/$REPO_NAME/pulls/$pr_number/requested_reviewers"' "$workflow"
...
assert_contains 'if ! run_listing="$(e2e_gh api' "$workflow"
assert_contains 'lifecycle observer unavailable' "$workflow"
assert_contains 'if ! labels="$(e2e_gh api' "$workflow"
assert_contains 'label observer unavailable' "$workflow"
assert_contains 'if ! run_state="$(e2e_gh run view' "$workflow"
assert_contains 'workflow status observer unavailable' "$workflow"
assert_contains 'if ! correlated_pr="$(e2e_gh api' "$workflow"
assert_contains 'reviewers[]=$MAINTENANCE_COPILOT_REVIEWER_LOGIN' "$workflow"
assert_contains '.head.sha == $sha' "$workflow"
...
assert_contains 'select(.conclusion != "skipped")' "$workflow"
...
assert_contains 'wait_for_run merge-maintenance-pr.yml pull_request_target "$pr_head_sha" "$branch" "$maintenance_started_at" "$PR_NUMBER" true' "$workflow"
...
assert_contains "wait_for_label 'automation: opt-in'" "$workflow"
