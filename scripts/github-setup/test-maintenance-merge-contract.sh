...
assert_contains "      - Quality" "$workflow"
assert_contains "      - Commit policy" "$workflow"
assert_contains "      - Test Quality Providers" "$workflow"
assert_contains "      - CodeQL Security Scan" "$workflow"
assert_contains "      - Test Generated Repository E2E" "$workflow"
...
assert_contains 'gh api graphql' "$workflow"
assert_contains 'enablePullRequestAutoMerge' "$workflow"
assert_contains 'pullRequestId' "$workflow"
assert_contains 'expectedHeadOid' "$workflow"
assert_contains 'native auto-merge request failed' "$workflow"
# shellcheck disable=SC2016  # literal workflow substring, not shell to expand
assert_not_contains 'pulls/$PR_NUMBER/auto-merge' "$workflow"
